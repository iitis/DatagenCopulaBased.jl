...
@testset "clayton chain copula" begin
  srand(43)
  x = chaincopulagen(500000, [-0.9, 3., 2], "clayton")
  ...
  srand(43)
  x = chaincopulagen(500000, [0.6, -0.2], "clayton"; cor = "pearson")
...
@testset "frank chain copula" begin
  srand(43)
  x = chaincopulagen(500000, [4., 11., 0.5, -12.], "frank")
  @test pvalue(ExactOneSampleKSTest(x[:,2], Uniform(0,1))) > α
  @test pvalue(ExactOneSampleKSTest(x[:,4], Uniform(0,1))) > α
  @test tail(x[:,1], x[:,2], "l", 0.0001) ≈ 0
  @test tail(x[:,4], x[:,3], "r", 0.0001) ≈ 0
  srand(43)
  x = chaincopulagen(500000, [0.8, 0.3, -0.5], "frank"; cor = "pearson")
  @test cor(x[:,1], x[:,2]) ≈ 0.8 atol=1.0e-3
  @test cor(x[:,2], x[:,3]) ≈ 0.3 atol=1.0e-3
end
@testset "Ali-Mikhail-Haq chain copulas" begin
  srand(43)
  x = chaincopulagen(500000, [0.3, 0.6, 1.], "amh")
  ...
  srand(43)
  x = chaincopulagen(500000, [0.45, 0.3], "amh"; cor = "pearson")
