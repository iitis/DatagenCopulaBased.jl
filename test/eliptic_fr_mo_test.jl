...
@testset "gaussian copula" begin
  ...
  Random.seed!(43)
  @test simulate_copula1(1, Gaussian_cop([1. 0.; 0. 1.])) ≈ [0.589188  0.817617] atol=1.0e-5
  Random.seed!(43)
  x = simulate_copula1(500000, Gaussian_cop([1. 0.5; 0.5 1.]))
...
@testset "t-student copula" begin
  ...
  Random.seed!(43)
  @test simulate_copula1(1, Student_cop([1. 0.; 0. 1.], 1)) ≈ [0.936433  0.983987] atol=1.0e-5
  ν = 10
  rho = 0.5
  λ = 2*pdf(TDist(ν+1), -sqrt.((ν+1)*(1-rho)/(1+rho)))
  Random.seed!(43)
  xt = simulate_copula1(500000, Student_cop([1. rho; rho 1.], ν))
...
@testset "Frechet copula" begin
  Random.seed!(43)
  @test simulate_copula1(1, Frechet_cop(4, 1)) ≈ [0.9248760  0.9248760  0.9248760  0.9248760] atol=1.0e-5
  Random.seed!(43)
  @test frechet(1, rand(1,4)) ≈ [0.9248760  0.9248760  0.9248760  0.9248760] atol=1.0e-5
  Random.seed!(43)
  @test simulate_copula1(1, Frechet_cop(2, 0.4, 0.4)) ≈ [0.1809752 0.7753771] atol=1.0e-5
  Random.seed!(43)
  @test frechet(0.4, 0.4, rand(1,2)) ≈ [0.1809752 0.7753771] atol=1.0e-5
  Random.seed!(43)
  x = simulate_copula1(500000, Frechet_cop(3, 0.3))
  @test pvalue(ExactOneSampleKSTest(x[:,1], Uniform(0,1))) > α
  @test pvalue(ExactOneSampleKSTest(x[:,2], Uniform(0,1))) > α
  ...
  x = simulate_copula1(500000, Frechet_cop(3, 1.))
  ...
  Random.seed!(43)
  x = simulate_copula1(1500000, Frechet_cop(2, .0))
  @test corspearman(x) ≈ [1. 0.; 0. 1.] atol=1.0e-2

  x = simulate_copula1(500000, Frechet_cop(2, 0.8, 0.1));
  @test pvalue(ExactOneSampleKSTest(x[:,1], Uniform(0,1))) > α
  @test pvalue(ExactOneSampleKSTest(x[:,2], Uniform(0,1))) > α
...
@testset "Marshall-Olkin copula" begin
  Random.seed!(43)
  @test simulate_copula1(1, Marshall_Olkin_cop([1., 2., 3.])) ≈ [0.854724  0.821831] atol=1.0e-5
  ...
  Random.seed!(43)
  x = simulate_copula1(100000, Marshall_Olkin_cop([1.1, 0.2, 0.6]))
  @test pvalue(ExactOneSampleKSTest(x[:,1], Uniform(0,1))) > α
  @test pvalue(ExactOneSampleKSTest(x[:,2], Uniform(0,1))) > α
  a2 = 0.6/0.8
  a1 = 0.6/1.7
  ...
  Random.seed!(42)
  x = simulate_copula1(100000, Marshall_Olkin_cop([1.1, 0.2, 2.1, 0.6, 0.5, 3.2, 7.1, 2.1]))
  @test pvalue(ExactOneSampleKSTest(x[:,1], Uniform(0,1))) > α
  @test pvalue(ExactOneSampleKSTest(x[:,2], Uniform(0,1))) > α
  @test pvalue(ExactOneSampleKSTest(x[:,3], Uniform(0,1))) > α
